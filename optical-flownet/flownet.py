...
def flownet_s(inputs, channel_mult=1, full_res=False):
    ...
    with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
        ...
        batch1 = slim.batch_norm(conv1, data_format='NHWC', scope="batch1")
        conv2 = slim.conv2d(batch1, int(128 * m), 5, stride=2, scope='conv2')
        batch2 = slim.batch_norm(conv2, data_format='NHWC', scope="batch2")
        conv3 = slim.conv2d(batch2, int(256 * m), 5, stride=2, scope='conv3')
        batch3 = slim.batch_norm(conv3, data_format='NHWC', scope="batch3")
        conv3_1 = slim.conv2d(batch3, int(256 * m), 3, stride=1, scope='conv3_1')
        batch3_1 = slim.batch_norm(conv3_1, data_format='NHWC', scope="batch3_1")
        conv4 = slim.conv2d(batch3_1, int(512 * m), 3, stride=2, scope='conv4')
        batch4 = slim.batch_norm(conv4, data_format='NHWC', scope="batch4")
        conv4_1 = slim.conv2d(batch4, int(512 * m), 3, stride=1, scope='conv4_1')
        batch4_1 = slim.batch_norm(conv4_1, data_format='NHWC', scope="batch4_1")
        conv5 = slim.conv2d(batch4_1, int(512 * m), 3, stride=2, scope='conv5')
        batch5 = slim.batch_norm(conv5, data_format='NHWC', scope="batch5")
        conv5_1 = slim.conv2d(batch5, int(512 * m), 3, stride=1, scope='conv5_1')
        batch5_1 = slim.batch_norm(conv5_1, data_format='NHWC', scope="batch5_1")
        conv6 = slim.conv2d(batch5_1, int(1024 * m), 3, stride=2, scope='conv6')
        batch6 = slim.batch_norm(conv6, data_format='NHWC', scope="batch6")
